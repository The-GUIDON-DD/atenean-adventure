 ...
 * Taken from https://stackoverflow.com/a/25349431
 */
function bounceTween (game, position, from, to, duration, bounceHeight, bounces, scaleX) {
    return new Promise(resolve => {
        ...
        move.onComplete.addOnce(() => {
            jump.stop();
            game.screenEffects.transition.CUT(from, to, position, scaleX).then(resolve);
        });
    })
}
...
class MoveWithBounce extends RenJS.Plugin {
    onInit(){
        this.game.screenEffects.transition['MoveWithBounce'] = (from, to, position, scaleX) => {
            ...
            return bounceTween(this.game, position, from, to, duration, bounceHeight, bounces, scaleX);
        }
    }
}
